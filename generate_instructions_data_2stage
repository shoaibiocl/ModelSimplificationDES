"""
2-Stage tandem queueing simulation (M/M/1, M/G/1, G/G/1) data generator

This script automates an experimental parameter sweep across server utilization
levels (20% to 92% in this case) to generate number of instructions per arrical for the DES runtime prediction framework.

Outputs:
    - Captures discrete-event trace caches via a custom salabim environment.
    - Extracts trace log event density metrics (surrogate for CPU load).
    - Exports results to an Excel matrix ('2StageMG1_v1.xlsx').
"""
